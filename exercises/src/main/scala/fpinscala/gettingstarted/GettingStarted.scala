...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    def go(n: Int, first: Int, second: Int): Int = {
      if (n <= 0) second
      else go(n-1, first + second, first)
    }
...
object TestSort {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val as_sorted = Array(1, 5, 5, 6, 9, 340, 567)
    val as_not_sorted = Array(4, 2, 5, 5, 8, 2, 6)

    println("[1] check sorted: " 
      + as_sorted.deep.mkString(" ") 
      + " result: %s"
        .format(
          PolymorphicFunctions.isSorted(as_sorted, (x: Int, y: Int) => x <= y )
        )
    )

    println("[2] check unsorted: "
      + as_not_sorted.deep.mkString(" ")
      + " result %s".format(
        PolymorphicFunctions.isSorted(as_not_sorted, (x: Int, y: Int) => x <= y)
      )
    )
